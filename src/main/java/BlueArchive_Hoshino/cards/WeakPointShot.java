...
public class WeakPointShot extends CustomCard implements ShotCard {
    ...
    public static final String ID = DefaultMod.makeID(WeakPointShot.class.getSimpleName());
    private static final CardStrings cardStrings = CardCrawlGame.languagePack.getCardStrings(ID);

    public static final String IMG = makeCardPath("WeakPointShot.png");
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        int vulnerable_power = BulletSubscriber.getBullet() - 1;
        this.addToBot(new BulletModifyAction(-1));
        AbstractDungeon.actionManager.addToBottom(
                new DamageAction(m, new DamageInfo(p, damage, damageTypeForTurn),
                        EnumPatch.HOSHINO_SHOTGUN));
        if(vulnerable_power > 0) {
            this.addToBot(new ApplyPowerAction(m, p, new VulnerablePower(m, vulnerable_power, false), vulnerable_power));
        }
